...
the large volume of textual prompts needed to get the relevent LLM's up and running.
...
Prompts are defined using the toml syntax under the 'prompt.' namespace,
...
class ParseError(Exception):
    """
    Emitted when the prompt parser chokes.
    """
...
class Prompt:
    ...
    with the keyword "prompt". Such classes are expected to contain
    ...
    def format(self, dynamic_dependencies: Dict[str, str]):
        """
        Formats the prompt, yielding the result
        :param dynamic_dependencies:
        :return:
        """

        # Check all depedencies are satisfied
    ...
    def say(self, dependencies: Dict[str, str])->Generator[str, None, None]:
        ...
        if self.max_width is None:
            # If there is no character limit, just yield
            yield prompt
        else:
            # Character limit exists. Wrap to under that limit.
            ...
            if num_sections == 1:
                # If we only have a one line wrapped feature, fantastic.
                # We just return it rather than using the full chopping mechanism
                yield lines[0]
            else:
                # Chop up the prompt and yield each section in sequence
...
def parse_prompt(parsing: str,
    ...
    for text, format_entry, _, _ in formatter.parse(parsing):
        ...
        if format_entry is not None:
            ...
            # Detect and throw on recursion. Store
            # used resources
            ...
            # Decide whether the resource is dynamic or static. Act appropriately
            if format_entry in resources:
                ...
                try:
                    ...
                    # Handle exception. Clearly indicate what we were trying to do at the
                    # time so the user can see when they misnamed something.
                ...
                Doublecheck you spelled your formatting references correct,
...
def load_prompt_from_dict(prompt_dict: Dict[str, str],
    ...
    :param prompt_dict: A prompt dictionary to parse. Must contain "{PROMPT_KEYWORD}" feature and may
                        contain various static formatting arguments. Also may contain a list
                        of dynamic keyword whitelists in '{DYNAMIC_KEYWORD}'
    ...
    :param dynamic_keywords: Dynamic keyword whitelist can be fed in here. We can also discover
           whitelist elements from the '{DYNAMIC_KEYWORD}' dictionary whitelist
...
def load_prompts_from_file(file,
    ...
    prompt namespace. Classes defined under this namespace are expected to have
    ...
    This is an example piece of TOML that illustrates all these capacitieis
    ...
    :param max_size: The maximum number of characters before we need to slice up a prompt
                     into pieces.
    :param debug: Prints debug information, including what
    :return: A dictionary of discovered prompts
    """
    with open(file, 'r') as f:
        config_contents = toml.load(f)
        prompts = config_contents['prompt']
