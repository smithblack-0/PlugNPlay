...
class testPrompt(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        #
        # The two features are dependency one and dependency two
        test_prompt = """
    ...
    def test_init(self):
        ...
        # Initialization should resolve the fields.
        #
        # We should end up with a template, a dependencies, and a
        # max width field being populated.
    ...
    def test_say(self):
        """
        Test the generator the prompter produces, and ensures we are folling all
        the rules we need to.
        :return:
        """

        # Test we yield the formatted prompt directly when working without
        # a character limit
        prompt = prompts.Prompt(self.test_template, None)
...
class test_load_prompt_from_dict(unittest.TestCase):
    """
    Load prompt from dict is one of the major functions.
    """
    def setUp(self):

        # Construct the test data that will be parsed
        test_data = {}
        prompt = """
        This is a test prompt.
        ...
        # Define dynamic allowed
        test_data["dynamic_keywords"] = ["dynamic", "dynamic_2"]

        # Construct expected parsed structures
    ...
    def test_dynamic_throw_not_on_whitelist(self):
        ...
        # Attempt this with no dynamic reference
        with self.assertRaises(prompts.ParseError) as err:
            prompt = prompts.load_prompt_from_dict(bad_data)
        if PRINT_ERROR_MESSAGES:
    ...
    def test_throw_referring_to_dynamic_list(self):
        ...
        with self.assertRaises(prompts.ParseError) as err:
            prompt = prompts.load_prompt_from_dict(resources)
        if PRINT_ERROR_MESSAGES:
    ...
    def test_throw_wrong_dtype(self):
        ...
        with self.assertRaises(prompts.ParseError) as err:
            output = prompts.parse_prompt(prompt, resources, used, {})
    ...
    def test_detect_recurrent_resource(self):
        ...
        resources = {"prompt" : "This refers to {recurrent}",
    ...
    def test_dynamic_pass(self):
        """Test if the dynamic passin works """
...
class test_load_prompts_from_file(unittest.TestCase):
    """
    ...
    def test_load(self):
        ...
        self.assertEqual(list(results.keys()), ["TestFeature", "DynamicTestFeature"])